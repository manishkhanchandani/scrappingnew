...
$cn = $_GET['cn'];
$limit = $_GET['limit']; if(!$limit) $limit = 1;
if($_GET['cns']) {
	$cns = explode(",",strtolower($_GET['cns']));
	$cns2 = "'".implode("', '", $cns)."'";
	echo $sql = "select * from property_xml where gotit = 0 and country IN (".$cns2.") and hotel_id != 0 and improvement4 = 0 limit $limit";
} else if($_GET['cn']) {
		...
		if(eregi("Sorry, but nothing matches your search", $content)) {
			...
			if(eregi("Location Results", $content)) {
				...
				if(!file_exists($firstfile)) {		
					...
					$input = $tmp2[0];
					$first = $Property->getBasePageParseResultImp($input, $firstfile, $rec);
					$found = $first['found'];
				} else {
					$first = $Property->getSavedPageParseResultImp($firstfile, $rec);
					$found = $first['found'];
				}
			}
			if(!$found) {
