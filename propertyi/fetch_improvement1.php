...
// improvement in the search where search is not based on city
$time_start = microtime(true);
$Property = new Property;
$limit = $_GET['limit']; if(!$limit) $limit = 20;
echo $sql = "select * from property_xml where flag = 1 and hotel_id != 0 and gotit = 0 and improvement1 = 0 limit $limit";
echo "<br>";
$rs = mysql_query($sql) or die(mysql_query().'<meta http-equiv="refresh" content="30" />');
if(mysql_num_rows($rs)) {
	while($rec = mysql_fetch_array($rs)) {
		// initial settings
		if(eregi($rec['city']."&", $rec['baseurl'])) {
			echo $sql = "update property_xml set improvement1 = 1 where id = '".$rec['id']."'";
