...
// improvement str_replace str_ireplace
$time_start = microtime(true);
$Property = new Property;
$limit = $_GET['limit']; if(!$limit) $limit = 25;
echo $sql = "SELECT * FROM property_xml WHERE improvement3 = 0 and gotit = 0  and flag = 1 limit $limit";
echo "<br>";
...
if(mysql_num_rows($rs)) {
	while($rec = mysql_fetch_array($rs)) {
		...
		echo $sql = "update property_xml set improvement3 = 1 where id = '".$rec['id']."'";
		...
		echo $baseurl = "http://www.tripadvisor.com/Search?q=".urlencode($rec['name'])."+".urlencode($rec['city'])."&geo=1&returnTo=__2F__&hid=&verbose=&hur=&ssrc=h";
		$result['base']['baseurl'] = $baseurl;
		echo $sql = "update property_xml set baseurl = '".$Property->clean($baseurl)."', baseflag = 1 where id = '".$rec['id']."'";
		echo "<br>";
