...
if($_GET['s']) {
	echo $sql = "select * from poii_xml where flag = 0 and poi_id != 0 limit $s,$limit";
} else if($_GET['cns']) {
	$cns = explode(",",strtolower($_GET['cns']));
	$cns2 = "'".implode("', '", $cns)."'";
	echo $sql = "select * from poii_xml where flag = 0 and country IN (".$cns2.") and poi_id != 0 limit $limit";
} else if($_GET['cn']) {
	echo $sql = "select * from poii_xml where flag = 0 and country = '".$cn."' and poi_id != 0 limit $limit";
} else if($_GET['id']) {
	echo $sql = "select * from poii_xml where id = '".$_GET['id']."'";
} else {
	echo $sql = "select * from poii_xml where flag = 0 and poi_id != 0 limit $limit";
}
echo "<br>";
	...
	while($rec = mysql_fetch_array($rs)) {
		...
		if($first['found']) {
			...
			if($result['reviews']) {
				...
				$ts = "update poii_xml set flag = 1, ftype = '".$poi->clean($first['ftype'])."', heading = '".$poi->clean($first['heading'])."', avgrating = '".$poi->clean($first['rating'])."', totalreview = '".$poi->clean($first['review'])."', otherpageurls = '".$poi->clean($otherpageurls)."', otherpageurlflag = '1', gotit = '".$poi->clean($gotit)."', reviewxml = ''  where id = '".$rec['id']."'";
				echo "<br>";
				echo $ts;
				mysql_query($ts) or die(mysql_error());
			}
